...
"""
    public static func decode(_ data: Data, validIDs: Set<String>) throws -> TrainingReview {
        guard data.count <= 24000, let obj = try JSONSerialization.jsonObject(with: data) as? [String: Any], Set(obj.keys) == Set(["summary","findings","suggestions","limitations","evidenceIDs"]) else { throw CloudVoiceError.message("評價格式不完整，請重試。") }
        let r = try JSONDecoder().decode(TrainingReview.self, from: data)
        guard !r.summary.isEmpty, r.summary.count <= 1000, (1...4).contains(r.findings.count), (1...3).contains(r.suggestions.count), (1...4).contains(r.limitations.count), (1...12).contains(r.evidenceIDs.count), Set(r.evidenceIDs).isSubset(of: validIDs), (r.findings + r.suggestions + r.limitations).allSatisfy({ !$0.isEmpty && $0.count <= 1600 }) else { throw CloudVoiceError.message("評價缺少有效依據，請重試。") }
        return r
    }
    ...
    public static func generate(context: String, ids: Set<String>, configuration: CloudVoiceConfiguration = .load()) async throws -> TrainingReview {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw CloudVoiceError.message("AI 評價暫時不可用；訓練已保留，可稍後重試。") }
...
extension TrainingInsights {
    public static func reviewContext(_ session: WorkoutSession) -> String {
        let energy = report(session)
        let history = (session.client?.sessions ?? []).filter { $0.id != session.id && !$0.isInProgress && $0.date < session.date }.sorted { $0.date > $1.date }.prefix(4)
        ...
        let obj: [String: Any] = ["language": L("Traditional Chinese", "English"), "goal": session.client?.goal ?? "unknown", "energy": (try? JSONSerialization.jsonObject(with: JSONEncoder().encode(energy))) ?? [:], "history": historical, "assumptions": assumptions, "missing": "No ordinary strength RPE, pain report, technique video, or measured strength duration. Historical data is not proof of comparable conditions."]
        return (try? JSONSerialization.data(withJSONObject: obj, options: [.sortedKeys])).flatMap { String(data:$0, encoding:.utf8) } ?? "{}"
    }
    public static func reviewKey(_ session: WorkoutSession) -> String {
        // Includes goal, language and history; edits invalidate a running response as well as cached prose.
        let data = Data(reviewContext(session).utf8)
