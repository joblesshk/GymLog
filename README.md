...
6. **动作库与模板**：内置约 300 个中英双语动作。训练模板、Superset 模板、WOD 模板分属三个互不重叠的库；WOD 模板内置多个公开知名的 CrossFit 基准 WOD（如 Fran、Grace、Murph）。在「今天」添加 Superset 或 WOD 时优先从对应模板库选取，模板中没有时可退回手动逐项添加。
